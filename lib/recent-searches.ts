 ...
 */
export type RecentSearch = { candidateNumber: string; year: string };
...
/** Adds/moves an entry to the front (deduped by number+year, scoped per year) and persists. Returns the new list so the caller can update its own state without a second read. */
export function saveRecentSearch(candidateNumber: string, year: string): RecentSearch[] {
  const current = loadRecentSearches();
  const next = [{ candidateNumber, year }, ...current.filter((entry) => !(entry.candidateNumber === candidateNumber && entry.year === year))].slice(0, MAX_ENTRIES);
